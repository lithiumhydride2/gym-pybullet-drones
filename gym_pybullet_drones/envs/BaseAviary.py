...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        if self.VISION_ATTR:
            ...
            if self.IMG_CAPTURE_FREQ % self.PYB_STEPS_PER_CTRL != 0:
                print(
                    "[ERROR] in BaseAviary.__init__(), PyBullet and control frequencies incompatible with the desired video capture frame rate ({:f}Hz)"
        ...
        self._action_space = self._actionSpace()
        self._observation_space = self._observationSpace()
        #### Housekeeping ##########################################
        self._housekeeping()
    ...
    def step(self, action, need_return=True):
        ...
        if self.RECORD and not self.GUI and self.step_counter % self.CAPTURE_FREQ == 0:
            ...
            if self.VISION_ATTR:
                for i in range(self.NUM_DRONES):
                    self.rgb[i], self.dep[i], self.seg[
                        i] = self._getDroneImages(i)
                    #### Printing observation to PNG frames example ############
                    self._exportImage(
                        img_type=ImageType.
                        RGB,  # ImageType.BW, ImageType.DEP, ImageType.SEG
    ...
    def render(self, mode='human', close=False):
        ...
        print(
            ...
            "simulation time {:.1f}s@{:d}Hz ({:.2f}x)".format(
                self.step_counter * self.PYB_TIMESTEP, self.PYB_FREQ,
    ...
    def _housekeeping(self):
        ...
        self.DRONE_IDS = np.array([
            p.loadURDF(pkg_resources.resource_filename('gym_pybullet_drones',
                                                       'assets/' + self.URDF),
                       self.INIT_XYZS[i, :],
    ...
    def _getAdjacencyMatrix(self):
        ...
        for i in range(self.NUM_DRONES - 1):
            for j in range(self.NUM_DRONES - i - 1):
                if np.linalg.norm(self.pos[i, :] -
                                  self.pos[j + i +
                                           1, :]) < self.NEIGHBOURHOOD_RADIUS:
    ...
    def _integrateQ(self, quat, omega, dt):
        ...
        if np.isclose(omega_norm, 0):
            return quat
        lambda_ = np.array([[0, r, -q, p], [-r, 0, p, q], [q, -p, 0, r],
                            [-p, -q, -r, 0]]) * .5
        theta = omega_norm * dt / 2
        quat = np.dot(
            np.eye(4) * np.cos(theta) +
            2 / omega_norm * lambda_ * np.sin(theta), quat)
        return quat
    ...
    def _normalizedActionToRPM(self, action):
        ...
        return np.where(
            action <= 0, (action + 1) * self.HOVER_RPM,
    ...
    def _parseURDFParameters(self):
        ...
        URDF_TREE = etxml.parse(
            pkg_resources.resource_filename('gym_pybullet_drones',
    ...
    def _calculateNextStep(self, current_position, destination, step_size=1):
        ...
        distance = np.linalg.norm(
            direction)  # Calculate the distance to the destination
